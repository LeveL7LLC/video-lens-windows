...
> **Windows:** `task`/`rsync` aren't available — use `pwsh -File install.ps1` (creates a `python3` shim if needed, installs deps, copies both skills to `~/.claude/skills`). The scripts are env-independent on Windows: they force UTF-8 I/O and invoke yt-dlp via `python -m yt_dlp`, so no `PYTHONUTF8` or yt-dlp-on-PATH tweaks are required. Raycast and the mlx-whisper fallback are macOS / Apple-Silicon only.
