...
license: MIT
compatibility: "Requires Python 3 and youtube-transcript-api >=0.6.3. Optional but recommended: yt-dlp and deno for enriched metadata and chapters. Local transcription fallback (videos without captions) additionally requires mlx-whisper, ffmpeg, and yt-dlp (Apple Silicon only). Runs on macOS, Linux, and Windows (Git Bash); on Windows a `python3` command must be available — the bundled install.ps1 creates a shim if needed."
