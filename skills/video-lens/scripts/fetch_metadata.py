...
# Windows portability: Python defaults the standard streams to the legacy cp1252
# code page, which raises UnicodeEncodeError on non-ASCII transcript or metadata
# text. Force UTF-8 so reports in any language work instead of crashing.
...
def _ytdlp_cmd():
    """Resolve a yt-dlp invocation that does not require yt-dlp on PATH.

    Prefer ``python -m yt_dlp`` (the pip module in the current interpreter — how
    Windows/pip installs it, often into a Scripts dir that is not on PATH), and
    fall back to a ``yt-dlp`` binary on PATH (e.g. a Homebrew install on macOS).
    """
...
def main():
    ...
    try:
        result = subprocess.run(
            [*_ytdlp_cmd(), "--skip-download", "--quiet", "--no-warnings",
             "--no-check-formats", "--dump-json", url],
            capture_output=True, text=True, encoding="utf-8", timeout=60,
        )
    except FileNotFoundError:
        print("ERROR:YTDLP_MISSING: yt-dlp not installed — run: pip install yt-dlp (or brew install yt-dlp)")
        sys.exit(0)
