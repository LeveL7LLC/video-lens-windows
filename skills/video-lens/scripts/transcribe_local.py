...
# code page, which raises UnicodeEncodeError on non-ASCII transcript text. Force
# UTF-8 so reports in any language work instead of crashing.
...
def _ytdlp_cmd():
    """yt-dlp invocation that does not require yt-dlp on PATH: prefer the pip
    module in the current interpreter, fall back to a ``yt-dlp`` binary."""
...
def _download_audio(video_id, tmp_dir):
    result = subprocess.run(
        [*_ytdlp_cmd(), "-f", "bestaudio[ext=m4a]/bestaudio",
         "--socket-timeout", "30",
...
def main():
    ...
    if not _ytdlp_available():
        ...
        print("ERROR:AUDIO_DOWNLOAD_FAILED: yt-dlp not installed — pip install yt-dlp (or brew install yt-dlp)")
        sys.exit(1)
